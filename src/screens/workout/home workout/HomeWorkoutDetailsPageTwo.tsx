...
const HomeWorkoutDetailsPageTwo = ({workout}) => {
  const {assets, colors, sizes} = useTheme();

  const keyTips = JSON.parse(workout.key_tips);
  const excecutions = JSON.parse(workout.steps);
    ...
    <>
      <Block card paddingHorizontal={sizes.sm} marginHorizontal={10} row>
        <Block row>
          ...
          <Block center>
            {workout.body_part.map((exercise) => (
        ...
        {keyTips.map((tip, index) => (
          <Block>
            <Block row>
              ...
              <Text bold top={10} paddingLeft={10} paddingBottom={10} paddingTop={10}>
                {tip}
              </Text>
            </Block>
          </Block>
...
export default HomeWorkoutDetailsPageTwo;
