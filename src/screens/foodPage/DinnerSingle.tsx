...
const DinnerSingle = ({route, navigation}) => {
  ...
  const handleAddFood = async (item) => {
    ...
    switch (mealType) {
      case 'dinner':
        try {
          await addDinnerItem(item, mealDetails);
          console.log('Dinner item added successfully');
  ...
  const totalEveningCalorie = dinnerItems.reduce(
    (acc, item) => acc + parseFloat(item.details.totalCalorie),
    0,
  );
  const totalDinnerCalories = totalEveningCalorie.toFixed(2);
  ...
  return (
    <Block safe scroll>
      <Block
        ...
        <CircularProgress
          value={totalDinnerCalories}
          radius={55}
          duration={2000}
          activeStrokeWidth={12}
          progressValueColor={'#ffff'}
          activeStrokeColor="#baabf9"
          maxValue={data.calories * 0.36}
